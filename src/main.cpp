...
void initStackProtection() {
  ...
  // Stack overflow protection is already enabled by esp_task_wdt_add(NULL) in initWatchdog()
  // ESP32 automatically detects stack overflows when watchdog is active
...
void checkMemoryUsage() {
  ...
  if (now - last_memory_check >= MEMORY_CHECK_INTERVAL) {
    ...
    Serial.printf("Memory Status: Free=%u bytes, Min Free=%u bytes, Total=%u bytes\n", 
...
void setup() {
  ...
  if (!cameraManager.begin(configManager.getJPEGQuality(), configManager.getDefaultResolution())) {
    Serial.println("WARNING: Camera initialization failed - continuing without camera");
    Serial.println("System will run in limited mode (no photo capture)");
    // Continue without camera - system can still provide status and other services
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    cameraManager.getResolutionString(configManager.getDefaultResolution(), resolution_str, sizeof(resolution_str));
